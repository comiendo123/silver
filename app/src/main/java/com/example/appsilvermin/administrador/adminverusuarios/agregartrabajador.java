...
public class agregartrabajador extends AppCompatActivity {//implements View.OnClickListener {

    EditText ci, nombre, apellidos, cargo, celular, passwordd;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_agregartrabajador);

        ci = this.findViewById(R.id.modmarca);
        nombre = this.findViewById(R.id.modnumero);
        apellidos = this.findViewById(R.id.ingresarapellidos);
        cargo = this.findViewById(R.id.cargodetalleusuario);
        celular = this.findViewById(R.id.celulardetalleusuario);
        passwordd = this.findViewById(R.id.ingresarcontrasena);



        ingresar=findViewById(R.id.botoningresarusuario);
    ...
    private void insertar(){
        ...
        String passwordd1=passwordd.getText().toString().trim();

        //  String horas1=horas.getText().toString().trim();

        ProgressDialog progressDialog=new ProgressDialog(this);
        if(ci1.isEmpty()){
            ci.setError("complete los campos");
        }
        else if(nombre1.isEmpty()){
            nombre.setError("complete los campos");
        }
        else if(apellidos1.isEmpty()){
            apellidos.setError("complete los campos");
        }else if(cargo1.isEmpty()){
            cargo.setError("complete los campos");
        }else if(celular1.isEmpty()){
            celular.setError("complete los campos");
        }else if(passwordd1.isEmpty()){
            passwordd.setError("complete los campos");
        }
        /*    else if(horas1.isEmpty()){
                horas.setError("complete los campos");
            }*/else{
            progressDialog.show();
            StringRequest request=new StringRequest(Request.Method.POST, "http://10.0.2.2:8000/api/insertarusuario", new Response.Listener<String>() {
                @Override
                ...
                protected Map<String, String>getParams() throws  AbstractMethodError{
                    ...
                    params.put("ciusuario",ci1);
                    params.put("nombres",nombre1);
                    params.put("apellidos",apellidos1);
                    params.put("cargo",cargo1);
                    params.put("celular",celular1);
                    params.put("passwordd",passwordd1);
...
/*        initiu();
    ...
    private void initiu() {
        ...
        ingresar = this.findViewById(R.id.botoningresarusuario);
...
/*    StorageDir /*directory*/
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.botoningresarusuario) {
            checkPermission();
        ...
        passwordd=this.findViewById(R.id.ingresarcontrasena);
        imagentrabajador=this.findViewById(R.id.imagentr);
        ingresar1=this.findViewById(R.id.botoningresarusuario);
        cancelar=this.findViewById(R.id.botoncancelaringreso);
        tomarfoto=this.findViewById(R.id.tomarfoto);
...
//INGRESA DATOS BASICOS
    ...
    private void insertar() {
        ...
        if(ci1.isEmpty()){ ci.setError("complete los campos"); }
            ...
            progressDialog.show();
            StringRequest request = new StringRequest(Request.Method.POST, "http://localhost:2000/usuario",
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
